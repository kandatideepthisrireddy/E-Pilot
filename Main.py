...
def preprocessDataset():
    ...
    Y = to_categorical(Y)
    pilot_X_train, pilot_X_test, pilot_y_train, pilot_y_test = train_test_split(
        pilot, Y, test_size=0.2)
    actuator_X_train, actuator_X_test, actuator_y_train, actuator_y_test = train_test_split(
        actuator, Y, test_size=0.2)
    physical_X_train, physical_X_test, physical_y_train, physical_y_test = train_test_split(
        physical, Y, test_size=0.2)
    ...
    text.insert(END, "All features found in dataset            : " +
                str(all_data.shape[1])+"\n")
    text.insert(END, "Total Pilot features found in dataset    : " +
                str(pilot.shape[1])+"\n")
...
def runAP2TD():
    ...
    if os.path.exists('model/physical_model.json'):
        ...
        lstm_physical.make_predict_function()
    else:
        ...
        lstm_physical.add(
        Dense(physical_y_train.shape[1], activation='softmax'))
        # compile LSTM model
...
def runAP2DH():
    ...
    if os.path.exists('model/actuator_model.json'):
        ...
        lstm_actuator.make_predict_function()
    else:
        ...
        # defining output layer for prediction
        lstm_actuator.add(
            Dense(actuator_y_train.shape[1], activation='softmax'))
        # compile LSTM model
...
def runDH2TD():
    ...
    if os.path.exists('model/pilot_model.json'):
        ...
        lstm_pilot.make_predict_function()
    else:
...
def graph():
    df = pd.DataFrame([['SVM', 'Sensitivity', sensitivity[0]], ['SVM', 'Specificity', specificity[0]],
                       ['Logistic Regression', 'Sensitivity', sensitivity[1]], [
                           'Logistic Regression', 'Specificity', specificity[1]],
                       ['AP2TD', 'Sensitivity', sensitivity[2]], [
                           'AP2TD', 'Specificity', specificity[2]],
                       ['AP2DH', 'Sensitivity', sensitivity[3]], [
                           'AP2DH', 'Specificity', specificity[3]],
                       ['DH2TD', 'Sensitivity', sensitivity[4]], [
                           'DH2TD', 'Specificity', specificity[4]],
                       ], columns=['Parameters', 'Algorithms', 'Value'])
    # df.pivot("Parameters", "Algorithms", "Value").plot(kind='bar')
    df_pivoted = df.pivot(index='Parameters', columns='Algorithms', values='Value')
...
font = ('times', 15, 'bold')
title = Label(
    main, text='E-Pilots: A System to Predict Hard Landing During the Approach Phase of Commercial Flights')
